...
function CardImageBookingPartnership(props) {
  let { id, image } = props;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteThis = async (id) => {
          try {
            const response = await fetch(`../api/booking-image?id=${id}`, {
              method: "DELETE",
            });
          ...
          <Form>
            <div className="form-row">
              <Col className="p-0 text-left ">
                <Input disabled type="text" defaultValue={`${image}`} />
              </Col>
            </div>
          </Form>
        </Col>
...
CardImageBookingPartnership.propTypes = {
  id: PropTypes.any,
  image: PropTypes.any,
};

export default CardImageBookingPartnership;
