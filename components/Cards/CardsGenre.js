import React, { useState } from "react";

// reactstrap components
import {
  Button,
  Card,
  Col,
  Form,
  Input,
  Modal,
  Row,
  Spinner,
} from "reactstrap";

import PropTypes from "prop-types";

import "../../assets/css/main/main.module.css";

import Swal from "sweetalert2";
import { useRouter } from "next/router";

function CardGenre(props) {
  let { id, genre } = props;

  const [loading, setLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [newGenre, setNewGenre] = useState(`${genre}`);

  const router = useRouter();

  const deleteGenre = async (id, genre) => {
    Swal.fire({
      title: `Apakah anda yakin ingin menghapus genre ${genre}?`,
      text: "Menghapus genre akan menghapus segala film yang berkaitan dengan genre ini!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      confirmButtonText: "Ya, hapus genre ini!",
    }).then((result) => {
      if (result.isConfirmed) {
        const deleteThisGenre = async (id, genre) => {
          try {
            const response = await fetch(`../api/genre?id=${id}`, {
              method: "DELETE",
            });
            const data = await response.json();
            if (data) {
              if (data.statusCode != 200) {
                Swal.fire("FAILED", `Genre ${genre} gagal dihapus`, "error");
              } else {
                await Swal.fire(
                  "OK",
                  `Genre ${genre} berhasil dihapus`,
                  "success"
                );
                router.reload(window.location.pathname);
              }
            }
          } catch (error) {
            Swal.fire("FAILED", "Data gagal di proses", "error");
          }
        };
        deleteThisGenre(id, genre);
      }
    });
  };

  const updateGenre = async (genre) => {
    try {
      const response = await fetch(`../api/genre`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          id: id,
          name_genre: newGenre,
        }),
      });
      const data = await response.json();
      if (data) {
        if (data.statusCode != 201) {
          Swal.fire("FAILED", `Genre ${genre} gagal diupdate`, "error");
          setLoading(false);
        } else {
          await Swal.fire(
            "OK",
            `Genre ${genre} berhasil diupdate menjadi ${newGenre}`,
            "success"
          );
          router.reload(window.location.pathname);
        }
      }
      setLoading(false);
    } catch (error) {
      Swal.fire("FAILED", "Data gagal di proses", "error");
      setLoading(false);
    }
  };

  return (
    <>
      <Card className="mb-2">
        <div className="py-3 px-2">
          <Row className="align-items-center">
            <Col>
              <span className="mb-0">{genre}</span>
            </Col>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#697aa3",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                setModalOpen(!modalOpen);
              }}
            >
              <span>Update</span>
            </Button>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#cc0000",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                deleteGenre(id, genre);
              }}
            >
              <span>Delete</span>
            </Button>
          </Row>
        </div>
      </Card>

      <Modal
        toggle={() => setModalOpen(!modalOpen)}
        isOpen={modalOpen}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Genre</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Nama Genre</h5>
                <Input
                  id="form-genre"
                  type="text"
                  defaultValue={`${genre}`}
                  onChange={(e) => {
                    setNewGenre(e.target.value);
                  }}
                />
              </Col>
            </div>
          </Form>
          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#FE7900",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              updateGenre(genre);
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>
    </>
  );
}

CardGenre.propTypes = {
  id: PropTypes.any,
  genre: PropTypes.string,
};

export default CardGenre;
