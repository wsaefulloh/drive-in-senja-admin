import React, { useEffect, useState } from "react";

// reactstrap components
import {
  Button,
  Card,
  CardBody,
  CardTitle,
  Col,
  Modal,
  Spinner,
} from "reactstrap";

import PropTypes from "prop-types";

import "../../assets/css/main/main.module.css";

import { fetchWrapper } from "../../helpers/fetch-wrapper";

function CardPartnership(props) {
  let { img, id, title, desc } = props;

  const [modalOpen, setModalOpen] = useState(false);
  const [imagePrivate, setImagePrivate] = useState([]);

  const getImagePrivate = async () => {
    const data = await fetchWrapper.get(
      `api/booking-partner/get-booking?private=${id}`
    );
    if (data) {
      console.log(data);
      setImagePrivate(data.data);
    }
  };

  useEffect(() => {
    getImagePrivate();
  }, []);

  return (
    <>
      <Col lg="6" style={{ paddingRight: "7.5px", paddingLeft: "7.5px" }}>
        <Card
          style={{
            backgroundImage:
              "url(" +
              `https://drive.google.com/uc?export=view&id=${img}` +
              ")",
            height: "250px",
            width: "100%",
            backgroundPosition: "center",
            backgroundSize: "cover",
          }}
        >
          <CardBody className="gradient__card__detail_film d-flex flex-column justify-content-between">
            <div
              className="w-100 d-flex justify-content-start align-items-end"
              style={{ height: "175px" }}
            >
              <div>
                <CardTitle
                  className="mb-2"
                  tag="h2"
                  style={{ color: "#ffffff" }}
                >
                  {title}
                </CardTitle>
                <div>{desc}</div>
              </div>
            </div>
            <div className="w-100">
              <Button
                style={{
                  color: "#ffffff",
                  width: "100%",
                  backgroundColor: "#FE7900",
                  border: "0",
                }}
                type="button"
                className="w-100 p-1"
                onClick={() => {
                  setModalOpen(true);
                }}
              >
                <span>More</span>
              </Button>
            </div>
          </CardBody>
        </Card>
      </Col>

      <Modal
        toggle={() => setModalOpen(!modalOpen)}
        isOpen={modalOpen}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3>{title}</h3>
          <div
            className="d-flex justify-content-center"
            style={{ flexWrap: "wrap" }}
          >
            {imagePrivate.map((val) => {
              return (
                <div
                  style={{
                    paddingLeft: "9px",
                    paddingRight: "9px",
                    paddingTop: "15px",
                  }}
                >
                  <img
                    alt="..."
                    src={`https://drive.google.com/uc?export=view&id=${val.url_image}`}
                    height="30px"
                    width="auto"
                  />
                </div>
              );
            })}
          </div>
          <Button
            style={{
              color: "#ffffff",
              backgroundColor: "#FE7900",
              border: "0",
              marginTop: "30px",
            }}
            type="button"
            className="px-3"
          >
            <span>Kirim Email</span>
          </Button>
        </div>
      </Modal>
    </>
  );
}

CardPartnership.propTypes = {
  img: PropTypes.any,
  title: PropTypes.string,
  id: PropTypes.any,
  desc: PropTypes.any,
};

export default CardPartnership;
