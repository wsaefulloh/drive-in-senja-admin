import React, { useEffect, useState } from "react";

// reactstrap components
import {
  Button,
  Card,
  Col,
  Form,
  Input,
  Modal,
  Row,
  Spinner,
} from "reactstrap";

import PropTypes from "prop-types";

import "../../assets/css/main/main.module.css";
import CardImageBookingPartnership from "components/Cards/CardsImageBookingPartnership.js";

import Swal from "sweetalert2";
import { useRouter } from "next/router";
import { fetchWrapper } from "../../helpers/fetch-wrapper";

function CardBookingPartnership(props) {
  let { id, name_private, url_image, url_email, description } = props;

  const [loading, setLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalOpenAdd, setModalOpenAdd] = useState(false);
  const [newData, setNewData] = useState({
    id: `${id}`,
    name_private: `${name_private}`,
    url_image: `${url_image}`,
    url_email: `${url_email}`,
    description: `${description}`,
  });
  const [dataValue, setDataValue] = useState({
    id_private: `${id}`,
    url: "",
    url_image: "",
  });
  const [imageData, setImageData] = useState([]);

  const router = useRouter();

  const getImageByID = async () => {
    const data = await fetchWrapper.get(`../api/booking-image?id=${id}`);
    if (data) {
      setImageData(data.data);
    }
  };

  const deleteData = async (id) => {
    Swal.fire({
      title: `Apakah anda yakin ingin menghapus data ini?`,
      text: "Anda tidak dapat mengembalikan data yang sudah hilang!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      confirmButtonText: "Ya, hapus ini!",
    }).then((result) => {
      if (result.isConfirmed) {
        const deleteThis = async (id) => {
          try {
            const response = await fetch(`../api/private-booking?id=${id}`, {
              method: "DELETE",
            });
            const data = await response.json();
            if (data) {
              if (data.statusCode != 200) {
                Swal.fire("FAILED", `Data gagal dihapus`, "error");
              } else {
                await Swal.fire("OK", `Data berhasil dihapus`, "success");
                router.reload(window.location.pathname);
              }
            }
          } catch (error) {
            Swal.fire("FAILED", "Data gagal di proses", "error");
          }
        };
        deleteThis(id);
      }
    });
  };

  const updateData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`../api/private-booking`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(newData),
      });
      const data = await response.json();
      if (data) {
        if (data.statusCode != 201) {
          Swal.fire("FAILED", `Data gagal diupdate`, "error");
          setLoading(false);
        } else {
          await Swal.fire("OK", `Data berhasil diupdate`, "success");
          router.reload(window.location.pathname);
        }
      }
      setLoading(false);
    } catch (error) {
      Swal.fire("FAILED", "Data gagal di proses", "error");
      setLoading(false);
    }
  };

  const addImage = async () => {
    setLoading(true);
    try {
      const response = await fetch(`../api/booking-image`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(dataValue),
      });
      const data = await response.json();
      if (data) {
        if (data.statusCode != 201) {
          Swal.fire("FAILED", "Lengkapi semua data!", "error");
          setLoading(false);
        } else {
          await Swal.fire("OK", "Data berhasil ditambahkan", "success");
          router.reload(window.location.pathname);
        }
      }
      setLoading(false);
    } catch (error) {
      console.log(error);
      Swal.fire("FAILED", "Data gagal di proses", "error");
      setLoading(false);
    }
  };

  useEffect(() => {
    getImageByID();
  }, []);

  return (
    <>
      <Card className="mb-2">
        <div className="py-3 px-2">
          <Row className="align-items-center mb-3">
            <Col>
              <h5 className="mb-0">{name_private}</h5>
            </Col>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#FE7900",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                setModalOpenAdd(!modalOpenAdd);
              }}
            >
              <span>Add</span>
            </Button>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#697aa3",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                setModalOpen(!modalOpen);
              }}
            >
              <span>Update</span>
            </Button>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#cc0000",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                deleteData(id);
              }}
            >
              <span>Delete</span>
            </Button>
          </Row>

          {imageData.map((val) => {
            console.log(val);
            return (
              <CardImageBookingPartnership
                id={val.id}
                image={val.url_imageOrigin}
              />
            );
          })}
        </div>
      </Card>

      <Modal
        toggle={() => setModalOpen(!modalOpen)}
        isOpen={modalOpen}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Data</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Nama Partnership</h5>
                <Input
                  type="text"
                  defaultValue={`${newData.name_private}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
                      name_private: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL Google Drive Image Partership</h5>
                <Input
                  type="text"
                  defaultValue={`${newData.url_image}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
                      url_image: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Email Address Partnership</h5>
                <Input
                  type="text"
                  defaultValue={`${newData.url_email}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
                      url_email: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Description Partnership</h5>
                <Input
                  type="text"
                  defaultValue={`${newData.description}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
                      description: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#FE7900",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              updateData();
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>

      <Modal
        toggle={() => setModalOpenAdd(!modalOpenAdd)}
        isOpen={modalOpenAdd}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpenAdd(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Add Data</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL Partner (boleh kosong)</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue.url}`}
                  onChange={(e) => {
                    setDataValue({
                      ...dataValue,
                      url: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL Google Drive Image Partner</h5>
                <Input
                  type="text"
                  defaultValue={`${dataValue.url_image}`}
                  onChange={(e) => {
                    setDataValue({
                      ...dataValue,
                      url_image: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>

          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#FE7900",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              addImage();
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Add</>
            )}
          </Button>
        </div>
      </Modal>
    </>
  );
}

CardBookingPartnership.propTypes = {
  id: PropTypes.any,
  name_private: PropTypes.any,
  url_image: PropTypes.any,
  url_email: PropTypes.any,
  description: PropTypes.any,
};

export default CardBookingPartnership;
