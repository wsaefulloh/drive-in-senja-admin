...
function CardTicketPlatform(props) {
  let { id, name_ticket, url_image, url_ticket } = props;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteThis = async (id) => {
          try {
            const response = await fetch(`../api/ticket-platform?id=${id}`, {
              method: "DELETE",
            });
  ...
  const updateData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`../api/ticket-platform`, {
        method: "PUT",
        headers: {
      ...
      <Card className="mb-2">
        <div className="py-3 px-2">
          <Row className="align-items-center">
            <Col>
              <span className="mb-0">{name_ticket}</span>
            </Col>
            <Button
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Nama Ticket</h5>
                <Input
                  type="text"
                  defaultValue={`${newData.name_ticket}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                ...
                <Input
                  type="text"
                  defaultValue={`${newData.url_ticket}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
...
CardTicketPlatform.propTypes = {
  id: PropTypes.any,
  name_ticket: PropTypes.any,
  url_image: PropTypes.any,
  url_ticket: PropTypes.any,
};

export default CardTicketPlatform;
