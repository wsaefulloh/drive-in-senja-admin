import React, { useState } from "react";

// reactstrap components
import {
  Button,
  Card,
  Col,
  Form,
  Input,
  Modal,
  Row,
  Spinner,
} from "reactstrap";

import PropTypes from "prop-types";

import "../../assets/css/main/main.module.css";

import Swal from "sweetalert2";
import { useRouter } from "next/router";

function CardLocation(props) {
  let {
    id,
    name_location,
    details_location,
    date_location,
    url_maps,
    url_iframe,
  } = props;

  const [loading, setLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [newLocation, setNewLocation] = useState({
    name_location: `${name_location}`,
    details_location: `${details_location}`,
    date_location: `${date_location}`,
    url_maps: `${url_maps}`,
    url_iframe: `${url_iframe}`,
  });

  const router = useRouter();

  const deleteLocation = async (id, name_location) => {
    Swal.fire({
      title: `Apakah anda yakin ingin menghapus lokasi ${name_location}?`,
      text: "Menghapus lokasi akan menghapus segala film yang berkaitan dengan lokasi ini!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      confirmButtonText: "Ya, hapus lokasi ini!",
    }).then((result) => {
      if (result.isConfirmed) {
        const deleteThis = async (id, name_location) => {
          try {
            const response = await fetch(`../api/location?id=${id}`, {
              method: "DELETE",
            });
            const data = await response.json();
            if (data) {
              if (data.statusCode != 200) {
                Swal.fire(
                  "FAILED",
                  `Lokasi ${name_location} gagal dihapus`,
                  "error"
                );
              } else {
                await Swal.fire(
                  "OK",
                  `Lokasi ${name_location} berhasil dihapus`,
                  "success"
                );
                router.reload(window.location.pathname);
              }
            }
          } catch (error) {
            Swal.fire("FAILED", "Data gagal di proses", "error");
          }
        };
        deleteThis(id, name_location);
      }
    });
  };

  const updateLocation = async (name_location) => {
    setLoading(true);
    try {
      const response = await fetch(`../api/location`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          id: id,
          name_location: newLocation.name_location,
          details_location: newLocation.details_location,
          date_location: newLocation.date_location,
          url_maps: newLocation.url_maps,
          url_iframe: newLocation.url_iframe,
        }),
      });
      const data = await response.json();
      if (data) {
        if (data.statusCode != 201) {
          Swal.fire(
            "FAILED",
            `Lokasi ${name_location} gagal diupdate`,
            "error"
          );
          setLoading(false);
        } else {
          await Swal.fire("OK", `Lokasi berhasil diupdate`, "success");
          router.reload(window.location.pathname);
        }
      }
      setLoading(false);
    } catch (error) {
      Swal.fire("FAILED", "Data gagal di proses", "error");
      setLoading(false);
    }
  };

  return (
    <>
      <Card className="mb-2">
        <div className="py-3 px-2">
          <Row className="align-items-center">
            <Col>
              <span className="mb-0">{name_location}</span>
            </Col>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#697aa3",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                setModalOpen(!modalOpen);
              }}
            >
              <span>Update</span>
            </Button>
            <Button
              className="m-1 border-0 py-1 px-3"
              style={{
                color: "#ffffff",
                backgroundColor: "#cc0000",
                borderRadius: "5px",
                fontSize: "10px",
              }}
              type="button"
              onClick={() => {
                deleteLocation(id, name_location);
              }}
            >
              <span>Delete</span>
            </Button>
          </Row>
        </div>
      </Card>

      <Modal
        toggle={() => setModalOpen(!modalOpen)}
        isOpen={modalOpen}
        centered
        fade={true}
        size="md"
      >
        <div className="justify-content-center text-center p-4">
          <img
            onClick={() => {
              setModalOpen(false);
            }}
            style={{
              position: "absolute",
              top: "8px",
              right: "8px",
              width: "35px",
              cursor: "pointer",
            }}
            src="data:image/png;base64,"
          ></img>
          <h3 className="m-0 mb-3 p-0">Update Location</h3>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Nama Lokasi</h5>
                <Input
                  type="text"
                  defaultValue={`${newLocation.name_location}`}
                  onChange={(e) => {
                    setNewLocation({
                      ...newLocation,
                      name_location: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">Detail Lokasi</h5>
                <Input
                  type="text"
                  defaultValue={`${newLocation.details_location}`}
                  onChange={(e) => {
                    setNewLocation({
                      ...newLocation,
                      details_location: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">
                  Tanggal Lokasi (ex : 20 - 31 Juni 2022)
                </h5>
                <Input
                  type="text"
                  defaultValue={`${newLocation.date_location}`}
                  onChange={(e) => {
                    setNewLocation({
                      ...newLocation,
                      date_location: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL Maps</h5>
                <Input
                  type="text"
                  defaultValue={`${newLocation.url_maps}`}
                  onChange={(e) => {
                    setNewLocation({
                      ...newLocation,
                      url_maps: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL iFrame</h5>
                <Input
                  type="text"
                  defaultValue={`${newLocation.url_iframe}`}
                  onChange={(e) => {
                    setNewLocation({
                      ...newLocation,
                      url_iframe: `${e.target.value}`,
                    });
                  }}
                />
              </Col>
            </div>
          </Form>
          <Button
            color="secondary"
            style={{
              color: "#ffffff",
              backgroundColor: "#FE7900",
              maxWidth: "150px",
            }}
            className="border-0"
            type="button"
            onClick={() => {
              updateLocation(name_location);
            }}
          >
            {loading == true ? (
              <>
                <div className="py-1 text-center align-content-center align-items-center">
                  <Spinner
                    as="span"
                    animation="grow"
                    size="sm"
                    role="status"
                    aria-hidden="true"
                  />
                </div>
              </>
            ) : (
              <>Update</>
            )}
          </Button>
        </div>
      </Modal>
    </>
  );
}

CardLocation.propTypes = {
  id: PropTypes.any,
  name_location: PropTypes.any,
  details_location: PropTypes.any,
  date_location: PropTypes.any,
  url_maps: PropTypes.any,
  url_iframe: PropTypes.any,
};

export default CardLocation;
