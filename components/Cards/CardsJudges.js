...
function CardJudges(props) {
  let { id, url_image, name_judges, description } = props;

  const [loading, setLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [newData, setNewData] = useState({
    id: `${id}`,
    name_judges: `${name_judges}`,
    url_image: `${url_image}`,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const deleteThis = async (id) => {
          try {
            const response = await fetch(`../api/judges?id=${id}`, {
              method: "DELETE",
            });
      ...
      <Card className="mb-2">
        <div className="py-3 px-2">
          <Row className="align-items-center">
            <Col>
              <span className="mb-0">{name_judges}</span>
            </Col>
            <Button
      ...
      >
        <div className="justify-content-center text-center p-4">
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                ...
                <Input
                  type="text"
                  defaultValue={`${newData.name_judges}`}
                  onChange={(e) => {
                    setNewData({
                      ...newData,
          ...
          <Form>
            <div className="form-row">
              <Col className="mb-3 p-0 text-left ">
                <h5 className="pl-1 mb-1">URL Google Drive Image Judges</h5>
                <Input
                  type="text"
